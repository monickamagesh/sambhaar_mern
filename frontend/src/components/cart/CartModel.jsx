...
const CartModel = ({ products, isOpen, onClose }) => {
  ...
  const handleCheckout = (e) => {
    e.stopPropagation();
    onClose(); // Close the CartModel
    navigate("/order-summary"); // Navigate to OrderSummary page
  };
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          transition: "transform 300ms cubic-bexier(0.25, 0.46, 0.45, 0.94)",
        }}
      >
        <div className="p-4 mt-4">
                  ...
                  <div className="flex flex-row md:justify-start justify-end items-center mt-2">
                    ...
                    <div className="ml-5">
                      ...
                      onClick={(e) => handleRemove(e, item._id)}>
                        <i className="ri-close-fill ri-sm text-gray-600"></i>
                      </button>
                    </div>
                  </div>
                </div>
          ...
          {products.length > 0 && <div>
            ...
            className="bg-red-500 px-3 py-1.5 text-white mt-2 rounded-md flex justify-between items-center mb-4">Clear cart</button>
            <button
            onClick={handleCheckout}
            className="bg-red-500 px-3 py-1.5 text-white mt-2 rounded-md flex justify-between items-center mb-4">Checkout</button>
        </div>
            </div>}
        </div>
      </div>
